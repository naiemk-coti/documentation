...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Web app wizard (recommended first)** | Guided flow and one-liner from [Networks](#networks). Use **tunnel** (`--with-frp`, COTI subdomain, internal Docker Nginx gateway + FRPC; no host TLS) or **own domain + host Nginx** (`--with-nginx`). | [**Installation**](installation.md), [**Wizard tunnel**](installation-wizard-tunnel.md), [**Own domain**](installation-own-domain.md), [**UI guide**](ui-guide/) |
...
Operators on the **manual** path can still earn **rewards** when they satisfy the same thresholds as wizard users (FQDN, reachability, uptime, holdings, etc.) — see the [**Installation**](installation.md) section, especially [**Manual full node setup**](manual-full-node.md).
...
* [**Wizard tunnel**](installation-wizard-tunnel.md) (`--with-frp`) — COTI subdomain, FRPC, internal Docker Nginx gateway; no host TLS.
